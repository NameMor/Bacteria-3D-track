function [Approx,D]=dwt1DforTrack(S,N)
...
%     Dtemp=zeros(len+2*l,1);
    ...
    epdl=25;%expand length
    fwline=polyfit(1:epdl,A(1:epdl),1);
    bwline=polyfit(1:epdl,A(len-epdl+1:len),1);
    foward=polyval(fwline,-l+1:0);
    backward=polyval(bwline,epdl+1:epdl+l);
%     foward=flipud(A(1:l));
